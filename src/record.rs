  ...
  device.build_input_stream(
    ...
    move |data: &[f32], _| {
      ...
      if local_peak >= vad_thresh {
        ...
        // Interrupt while the agent is audible, or a reply is in flight but
        // has not made a sound yet (still being generated by the LLM, or its
        // first phrase still being synthesized). `playback_active` cannot be
        // used for the audible half of this: it drops every time the queue
        // empties between chunks, so speaking in one of those gaps (which is
        // most of the start of a phrase) was silently ignored.
        let agent_audible = crate::state::GLOBAL_STATE
          .get()
          .map(|st| {
            st.playback.speaking.load(Ordering::Relaxed)
              || st.reply_in_flight.load(Ordering::Relaxed)
          })
