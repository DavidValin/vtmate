...
//
//  Tool calling (`stream_response_into_with_tools`) is a separate, narrower
//  path: it only exists today for `ollama` and `llama-server`, talked to
//  directly over their `/v1/chat/completions` / `/api/chat` endpoints rather
//  than through the `llm` crate, which has no tool-call support wired up
//  here yet. Every other provider falls back to the plain streaming path
//  with tools silently unavailable.
// ------------------------------------------------------------------
...
/// Provider names `stream_response_into_with_tools` will hand to the raw
/// local tool-calling path below instead of the `llm` crate. Kept narrow and
/// separate from `LOCAL_PROVIDERS`: `openai-compatible-api` is local too but
/// has no candidate endpoint shape wired up for tool calls yet.
fn supports_tool_calls(provider: &str) -> bool {
  matches!(provider.trim().to_lowercase().as_str(), "ollama" | "llama-server")
}

/// Like `stream_response_into`, but lets the model call tools mid-reply.
/// Only `ollama` and `llama-server` actually support this today (see the
/// module doc comment) - every other provider, or a call with no tools,
/// falls back to the plain streaming path and `on_tool_call`/`on_reasoning`
/// simply never fire.
pub async fn stream_response_into_with_tools(
  messages: &[crate::conversation::ChatMessage],
  target: &LlmTarget,
  interrupt_counter: Arc<AtomicU64>,
  expected_interrupt: u64,
  on_piece: &mut dyn FnMut(&str),
  tools: &[String],
  on_tool_call: Option<&mut dyn FnMut(&serde_json::Value)>,
  on_reasoning: Option<&mut dyn FnMut(&str)>,
  think: bool,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  if tools.is_empty() || !supports_tool_calls(&target.provider) {
    return stream_response_into(messages, target, interrupt_counter, expected_interrupt, on_piece)
      .await;
  }
...
/// Stream response from Llama/Ollama endpoints directly over their raw HTTP
/// chat-completions API, with fallback between endpoint shapes and
/// mid-stream cancellation support. When `include_tools` is true, the LLM
/// may return tool_calls which are delivered via `on_tool_call`. Reasoning
/// tokens (from models like Gemma 4) are delivered via `on_reasoning`.
///
/// This bypasses the `llm` crate entirely: it has no tool-call support to
/// build on yet, so this talks to `/v1/chat/completions` / `/api/chat`
/// by hand instead. Only reached through `stream_response_into_with_tools`
/// above, for `ollama` and `llama-server`.
async fn llama_server_stream_response_into(
  messages: &[crate::conversation::ChatMessage],
  llama_host: &str,
  llama_model: &str,
  server_type: &str,
  ...
  fn candidates(host: &str, server_type: &str) -> Vec<(String, ApiKind)> {
    let base = base_url_with_scheme(host);
...
mod tests {
  ...
  #[test]
  fn tool_calls_are_only_supported_locally() {
    assert!(supports_tool_calls("ollama"));
    assert!(supports_tool_calls("Llama-Server"));
    assert!(!supports_tool_calls("openai-compatible-api"));
    assert!(!supports_tool_calls("anthropic-api"));
  }
